...
import (
	...
	"github.com/mengzhuo/go-kcapi/sha3_384"
)

func TestSHashSHA3_384(t *testing.T) {
	...
	cmd := exec.Command("openssl", "dgst", "-sha3-384")
...
func TestSHashSHA3_384File(t *testing.T) {
	kh, err := sha3_384.New()
	if err != nil {
		t.Skip(err)
	}
	...
	out, err := exec.Command("openssl", "dgst", "-sha3-384", tf.Name()).Output()
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkSHashSHA3_384File(b *testing.B) {
